...
static inline float cx() { return spawnX; }
static inline float cy() { return spawnY; }

// ── vertex helpers ──────────────────────────────────────────────────────────
// addVertex records the point for ScanLineFill and passes it to OpenGL.
static void addVertex(float x, float y) {
    // Reduce size by scaling around the spawn center (50% size) //
    x = spawnX + (x - spawnX) * 0.5f;
    y = spawnY + (y - spawnY) * 0.5f;
